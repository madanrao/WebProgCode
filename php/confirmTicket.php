...
if($time==$mt_id['time_slot'])
{
	if($mt_id['total_seats']>=$seats){
       $cost=$seats*10;
		echo "<p>Date: <input type='text' id='datepicker'>&nbsp;&nbsp;&nbsp;&nbsp; <span='cool-style'>Cost:$".$cost."</span></p>";
		echo "<button id='billConfirm' class='btn btn-lg btn-primary'>Generate Bill</button>";
		
	}
	elseif ($mt_id['total_seats']!=0 && $mt_id['total_seats']<$seats){
		echo "<p>Only ".$mt_id['total_seats']."SEATS available.</p>";
		echo "<a href='bookMovie.php'>BACK</a>";
	}
	elseif ($mt_id['total_seats']==0){
